...
const FriendCard = ({ data }) => {
  return (
    <Link href={`/${data.id}`} className="card bg-base-100 text-center shadow-sm p-6">
      ...
      <div className="card-body">
        <h2 className="text-center text-black font-bold text-2xl">
          ...
          <div className="">{data.days_since_contact} day ago</div>
        ...
        <div className="text-center flex flex-col gap-6 justify-center items-center">
          ...
          <div className={`badge bg-[#EF4444] text-white ${data.status === "active" ? "bg-red-500" : data.status ==="overdue" ? "bg-green-500" : ""}`}>{data.status}</div>
        </div>
      </div>
    </Link>
  );
