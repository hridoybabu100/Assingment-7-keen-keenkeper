  ...
  return (
    <div className="max-w-[90%] lg:max-w-[70%] mx-auto my-10">
      ...
      {call.length === 0 ? (
        ...
        <div className="mt-6">
          {call.map((item) => (
            <div key={item.id} className="bg-base-300 p-6 rounded-xl mb-5 flex gap-4">
              <Image src={CallImg} width={30} height={10} alt="call img" />
              <div>
                <h2>
                  <span className="text-2xl text-black font-bold">Calling with</span>{" "}
                  {item.name}
                </h2>
          ...
          <div className="mt-6">
            {text.map((item) => (
              <div key={item.id} className="bg-base-300 p-6 rounded-xl mb-5 flex gap-4">
                <Image src={TextImg} width={30} height={10} alt="text img" />
                <div>
                  <h2>
                    <span className="text-2xl text-black font-bold">Messaging with</span>{" "}
                    {item.name}
                  </h2>
                  <p className="text-[#64748B]">March {item.next_due_date}</p>
                </div>
          ...
          <div className="mt-6">
            {vedio.map((item) => (
              <div key={item.id} className="bg-base-300 p-6 rounded-xl mb-5 flex gap-4">
                <Image src={VedioImg} width={30} height={10} alt="video img" />
                <div>
                  <h2>
                    <span className="text-2xl text-black font-bold">Video Calling with</span>{" "}
                    {item.name}
                  </h2>
                  <p className="text-[#64748B]">March {item.next_due_date}</p>
                </div>
